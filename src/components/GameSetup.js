...
function GameSetup({game, socket}) {
  ...
  return (
    <>
      ...
      <div>
        {!nameChosen && <p className="game-status">{game.playersJoined === game.totalPlayers ? 'All players joined. Pick a famous name to pass.' : `${game.playersJoined} / ${game.totalPlayers} players have joined. Waiting on all players to join.`}</p>}
        {nameChosen && <p className="game-status">{`You passed ${nameToGuess}. Waiting for ${game.totalPlayers - game.totalNamesSet} players to choose a name.`}</p>}  
      </div>
    </>
  )
}
